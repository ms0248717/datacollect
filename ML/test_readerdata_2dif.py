...
model = load_model('./dif_feature/C2_phase_rssi_dis.h5')
...
for ang in range(0, 3):
    ...
    for dis in range(0, 3):
        ...
        for lab in range(0, 4):
            ...
            result = [0, 0]
            for j in range(1, 21):
                phase = read_data('../reader_data/ML_realdata/sm_dis_ang/phase_' + LABELS[lab] + '_' + ANG[ang] + '_' + DIS[dis] + '_' + str(j) + '.csv')
                rssi = read_data('../reader_data/ML_realdata/sm_dis_ang/rssi_' + LABELS[lab] + '_' + ANG[ang] + '_' + DIS[dis] + '_' + str(j) + '.csv')
                ...
                for i in range(0, size):
                    A = np.hstack((X_phase[:,i][:,np.newaxis], X_rssi[:,i][:,np.newaxis], X_distance[:,i][:,np.newaxis]))
                    if i == 0:
                        X_test_A = A
                    else:
