...
for lab in range(0, 4):
    ...
    for j in range(1, 11):
        ...
        rssi = read_data('../reader_data/ML_realdata/gc_tagsize/rssi_' +
                        LABELS[lab] + '_h1o1_2_5_' + str(j) + '.csv')
        ...
        sum_y_pred_test = y_pred_test[0]
        for i in range(1, size):
            sum_y_pred_test = sum_y_pred_test + y_pred_test[i]
        max_y_pred_test = np.argmax(sum_y_pred_test)
        # print(sum_y_pred_test)
        # sum_y_pred_test = y_pred_test
        # for i in range(0, size/2):
        #    sum_y_pred_test[i] = y_pred_test[i] + y_pred_test[i+5]
        ...
        result[max_y_pred_test] = result[max_y_pred_test] + 1
